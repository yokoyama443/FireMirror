			...
			if r.URL.Path == server.Path {
				...
				if strBody != "" {
					...
					if checkForMaliciousContent(strBody, signatures) {
						...
						blockList, err := models.GetRecentBlockListByIP(ip, 1*time.Hour)
						if err != nil {
							blockList = models.BlockList{IP: ip}
							err = blockList.Insert()
						} else {
							err = blockList.Update()
						}
						if err != nil {
							log.Printf("Error updating block list: %v", err)
						}

						if blockList.Count >= 3 {
							_, err := rdb.Publish(ctx, "waf-attacker", ip).Result()
							if err != nil {
								fmt.Println(err)
							}
							fmt.Printf("Published message at %s : %s\n", "waf-attacker", ip)
						}
						return
					} else {
						fmt.Println("良性通信")
